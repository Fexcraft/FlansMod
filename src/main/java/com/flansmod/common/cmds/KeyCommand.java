...
public class KeyCommand extends CommandBase {
	...
	public void execute(MinecraftServer server, ICommandSender sender, String[] args) throws CommandException {
		...
		if(sender.getCommandSenderEntity().getRidingEntity() instanceof EntitySeat == false){
		...
		if(args.length < 1){
			...
			boolean bool = ((EntitySeat)sender.getCommandSenderEntity().getRidingEntity()).driveable.driveableData.hasLock = !((EntitySeat)sender.getCommandSenderEntity().getRidingEntity()).driveable.driveableData.hasLock;
