...
public class EntitySeat extends Entity implements IControllable, IEntityAdditionalSpawnData
{
	...
	public void addPassenger(Entity passenger){
		...
		//Print.debug("AP => " + new Date().getTime() + " " + (worldObj.isRemote ? "[CLIENT]" : "[SERVER]"));
	}
	...
	public void setDead(){
		if(worldObj.isRemote){
			this.isDead = true;
			//Print.debug("DD => " + new Date().getTime() + " [CLIENT] OK;");
		}
		else{
			this.isDead = true;
			//Print.debug("DD => " + new Date().getTime() + " [SERVER]");
		}
	}
